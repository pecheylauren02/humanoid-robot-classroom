...
1. Ask the robot to deliver an item
   ...
   {GREEN}deliver <item> from <source> to <destination>{RESET}
...
6. View recent robot tasks
   {YELLOW}COMMAND{RESET}:
   {GREEN}view log{RESET}

7. View all robot tasks
   {YELLOW}COMMAND{RESET}:
   {GREEN}view full log{RESET}
...
def main():
    ...
    print(
          f"\n🤖{YELLOW} Hello{RESET} Teacher, and welcome! I am your "
          f"{GREEN}HUMANOID CLASSROOM ROBOT{RESET}, ready to assist you today."
    )
    ...
    while True:
        ...
        if teacher_name and re.fullmatch(r"[A-Za-z ]+", teacher_name):
            break
        print(f"\n{RED}OOPS!{RESET}Enter a valid name, only letters!")

    print(
        f"\nWonderful to meet you, {CYAN}{teacher_name}{RESET}! "
        "I am fully operational and eager to help in your classroom."
    )
    print(
        f"\nI can assist with tasks like {GREEN}delivering items{RESET}, "
        f"{RED}monitoring classroom temperature{RESET}, and "
        f"{YELLOW}greeting students{RESET}."
    )
    ...
    print(
        f"\nAll set, {CYAN}{teacher_name}{RESET}! "
        "Type a command and I will get right to work:\n"
    )
    ...
    while True:
        try:
            ...
            print(
                f"\nIt seems you are leaving, {CYAN}{teacher_name}{RESET}. "
                "I hope I was able to help you! Goodbye 👋"
            )
        ...
        if verb == "exit":
            while True:
                confirm = input(
                    f"\n{CYAN}{teacher_name}{RESET}, {YELLOW}sure{RESET} "
                    "you want me to power down? (y/n): "
                ).strip().lower()

                if confirm in ["y", "yes"]:
                    print(
                        f"\nMission complete, {CYAN}{teacher_name}{RESET}! "
                        "I’m going to recharge. See you next time! 🤖⚡\n"
                    )

                    return  # exit main()
                elif confirm in ["n", "no"]:
                    print("\nPhew! Still here. Ready for your next command!\n")
                    break  # return to main loop
                else:
                    print(
                        f"\n{YELLOW}⚠️ ALERT{RESET}! Enter a valid response: "
                        f"{GREEN}y{RESET} or {RED}n{RESET}."
                    )

        elif verb == "help":
            print(f"\nHere you go, {CYAN}{teacher_name}{RESET}:")
            print(COMMANDS_GUIDE)
            ...
            if "Delivered" in result:
                ...
                # Forbidden item
                print(
                    f"\n{RED}WARNING{RESET}, {CYAN}{teacher_name}{RESET}! "
                    f"{result}\n"
                )
                ...
                print(
                    ...
                    f"{CYAN}{teacher_name}{RESET}. Let's try again.\n"
                )
            ...
            if issue:
                print(
                    f"\n{YELLOW}⚠️ ALERT{RESET}, {CYAN}{teacher_name}{RESET}! "
                    f"The temperature is {temp}°C — outside my safe range.\n"
                )

            else:
                print(
                    f"\nThe classroom temperature is {GREEN}{temp}{RESET}°C. "
                    f"Optimal for learning, {CYAN}{teacher_name}{RESET}!\n"
                )
            ...
            if last:
                action, who = last
                print(f"\nAction undid: {action} for {who or 'robot'}.\n")
            else:
                print(
                    f"\nNothing to undo, {CYAN}{teacher_name}{RESET}. "
            ...
            verb == "view"
            and len(parts) >= 3
            and parts[1].lower() == "full"
            and parts[2].lower() == "log"
        ):
            print("\n--- Full Robot Task Log ---")
            if robot.task_log:
